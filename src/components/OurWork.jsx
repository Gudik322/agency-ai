...
const OurWork = () => {
  ...
  return (
    <motion.div initial="hidden" whileInView="visible" viewport={{once:true}} transition={{staggerChildren: 0.2}} id="our-work" className='flex flex-col items-center gap-8 px-4 sm:px-12 lg:px-24 xl:px-40 pt-30 text-gray-700 dark:text-white'>
                ...
                workData.map((work, index)=>(
                    <motion.div initial={{opacity: 0, y:30}} whileInView={{opacity: 1, y:0 }} transition={{duration: 0.5, delay: index * 0.2}} viewport={{once: true}}
                     key={index} className='hover:scale-102 duration-500 transition-all cursor-pointer'>
                        ...
                        <h3 className='mt-3 mb-2 tex-lg font-semibold'>{work.title}</h3>
